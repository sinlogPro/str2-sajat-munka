...
*/
const initState = () => {
    // matrix = Array(rows).fill(Array(cols).fill(null)); Ez nem működött.
    matrix = Array(cols)
        .fill(null)
        .map(item => Array(rows).fill(null));
}
...
const setGameStage = () => {
    const gameStage = document.querySelector('.tictactoe');
    gameStage.textContent = '';
    for (let i = 0; i < rows; i++) {
        for (let j = 0; j < cols; j++) {
            const div = document.createElement('div');
            // div.innerText = `null`
            div.setAttribute("data-cell", j);
            div.setAttribute("data-row", i);
            div.setAttribute("class", "cell");
    
            gameStage.appendChild(div);
        }
    }
...
*/
const deleteSigns = () => {
    allCells.forEach(el => el.textContent = '');
}
...
const addClickListener = () => {
    allCells.forEach(el => el.addEventListener('click', handleClick));
}
...
const removeAllClickListeners = () => {
    allCells.forEach(el => el.removeEventListener('click', handleClick));
}
...
const checkWinner = () => {
    ...
    console.log(`checkColumnValues: ${checkColumnValues()}  |  checkDiagonalValues: ${checkDiagonalValues()}`);
    /*
    ...
    */
    if (checkValues(matrix) || checkColumnValues() || checkDiagonalValues()) {
        const message = `The winner is Player ${mark === 'X' ? 'O' : 'X'}.`;
        endGame(message);
    }
